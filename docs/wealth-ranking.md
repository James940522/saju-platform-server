> 현재 v8 설정: [상품별 프롬프트 관리와 5분 실행 예산](./wealth-ranking-prompt-v8.md). 아래에는 이전 구현 기록이 포함되어 있습니다.

> 2026-09-14 v7: 새 비교 풀이는 1위와 마지막 순위 두 사람만 해석하며 AI가 제목을 생성합니다. 공개 결과에서 참여자를 현재 이름으로 부릅니다. [현재 계약과 과거 결과 처리](./reading-jobs.md#재물운-비교-풀이와-ai-제목-v7)를 참고하세요.

> 현재 프론트엔드는 [백그라운드 풀이 작업 API](reading-jobs.md)를 사용합니다. 아래 동기 API는 기존 consumer 호환용으로 유지됩니다.

# 무료 재물운 랭킹

> 현재 동작: [세 자료 조합 및 포츈텔러 분석](./fortuneteller-analysis.md), [KASI 연결](./wealth-ranking-kasi.md). 아래 내용은 초기 구현 기록을 포함한다. 현재 프롬프트는 v8이며 설치된 fortuneteller 원본 분석을 개인별 전체/부분 범위로 사용한다. 아래 과거 자체 구현 설명은 현재 실행 경로가 아니다.
...
새 풀이 접수와 아래 호환용 동기 API에는 로그인이 필요하다. 저장된 결과는 별도 공개 API에서 로그인 없이 이름·순위·운세·비교 설명만 조회한다([현재 결과 계약](./reading-jobs.md)). 2~5명은 기존 서버 상품 카탈로그와 같은 초기 한도다. 동명이인은 허용하며 `chartId`로 구분한다. 같은 차트 또는 같은 프로필의 서로 다른 과거 차트를 중복 참여자로 넣을 수 없다. 새 인원수 정책이 필요하면 요청/응답 schema와 상품 카탈로그를 함께 변경한다.
...
비스트리밍·`include_thoughts: false`로 JSON 결과를 한 번 요청한다. JSON 요청 방식은 모델 config를 따르며 최종 출력은 서버가 검증한다. 검색/도구는 사용하지 않는다. 전체 응답 수신까지 기본/최대 300초(설정 범위 1초~300초), 응답 본문 64KiB 제한을 적용한다. AI 전용 Undici Agent의 별도 headers/body 시간 제한은 해제하고, 설정값의 AbortSignal로 전체 시간을 제한한다. 공공 API와 다른 외부 연결의 제한시간은 유지한다. 잘린 출력, 거절, 잘못된 JSON, Provider 실패는 성공 결과로 대체하지 않는다. 토큰 과금 상한을 보장하는 구현은 아니며 Provider 계정의 비용 제한과 실제 호출 측정이 필요하다.
...
기존 오류 필터에 따라 5xx 응답은 일반 메시지와 `data: null`로 Provider 상세를 숨긴다. 서버에는 요청 ID, 고정된 실패 단계, HTTP 상태만 진단 로그로 남기며 이름·생년월일·키·Provider 본문은 기록하지 않는다. 실패 단계별 확인 방법은 [재물운 KASI 연결 문서](./wealth-ranking-kasi.md#502-오류-진단)를 따른다.
...
신규 사주 저장은 기존 멱등 키로 재시도하고, 이미 추가한 참여자의 차트를 재사용한다. AI 생성은 자동 재시도하지 않는다. Frontend 요청 제한시간은 11분으로 서버의 최대 10분에 대조·전송 여유 1분을 더한다. 중복 클릭·동시 저장/생성을 막고, 페이지 이동/계정 변경 시 요청을 취소하며 이전 계정의 결과를 표시하지 않는다. 저장된 사주는 유지되지만 참여자 선택과 랭킹 결과는 새로고침 후 유지하지 않는다. 상세 흐름은 Frontend의 `docs/wealth-ranking-flow.md`에 기록했다.
