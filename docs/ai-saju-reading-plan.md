...
> 2026-09-12 구현 우선순위 변경: 사용자 요청에 따라 첫 AI 기능은 **무료 재물운 랭킹**이다.
> 전용 프롬프트, Kie adapter, 인증된 생성 API와 출력 검증의 실제 계약은
> [재물운 랭킹 구현 문서](./wealth-ranking.md)를 따른다. 아래 종합 풀이 우선 순서와
> 비동기 저장 설계는 이전 기획안이며 이번 무료 랭킹의 동기 생성 구현과 구분한다.

작성일: 2026-09-10. 개정: 2차 통합안. 상태: AI 풀이 논의용 초안. AI API·모델·프롬프트는 아직 구현되지 않았다. 입력 폼의 만세력 계산 API는 별도 [만세력 미리보기 문서](./saju-chart-preview.md)를 따른다.
...
| 입력 중 만세력 미리보기 API | `POST /v1/saju-charts/preview` 실제 계산과 폼 내 원국 표시 연결. 저장·AI는 기존 프로필 API와 `chartId` 경계를 유지 |
...
AI 개발은 두 단계로 구분한다. 먼저 합성 차트 fixture로 `ReadingContext → AI → 검증된 종합 리포트`를 로컬에서 평가한다. 다음으로 같은 생성 코드를 기존 서비스의 인증·차트 저장·풀이 생성/조회 흐름에 연결한다. 입력 중 만세력 미리보기 API에는 AI 생성 기능을 연결하지 않는다.
...
| 계산 정책 | `kr-mean-solar-midnight-v2`, `Asia/Seoul`, 보정 시각의 자정, 한국시 보정(127.5°, 균시차 제외, 역사적 표준시·서머타임 반영) |
...
출생지별 보정을 확대할 때에는 입력 계약과 계산 정책을 별도로 갱신하고 새 차트를 생성한다. AI 프롬프트 안에서 시간을 임의 보정하지 않는다. 현재 schema에는 `near_solar_term_boundary` warning 코드가 있지만 계산기가 실제로 이 warning을 생성하지는 않는다. 경계 근접 안내가 필요하면 기준과 계산을 추가해야 한다.

첫 연결에는 기존 테스트의 양력·정확한 시각 사례를 사용하되, 음력·윤달·시간 미상은 첫 서비스 공개 전 평가에 포함한다. 시간 미상 계산기가 내부적으로 12시를 사용하는 것은 절입일 확인 후 시주·대운을 제거하기 위한 구현이다. 정오에 태어났다는 정보나 8자 결과로 AI에 보내지 않는다.

현재 `luckCycleGender`는 프로필·Snapshot의 필수 계약이다. 기본 원국 풀이에 불필요하다는 이유로 이 기획에서 API·DB를 변경하지 않는다. 대운을 쓰지 않는 AI 입력에서는 제외하고, 입력 자체를 선택값으로 바꾸려면 이후 별도 계약 변경으로 검토한다. 현재 계약은 출생 국가·도시를 받지 않으므로 해외 출생 지원을 주장하거나 AI가 출생지를 추정하게 하지 않는다. 한국시 보정의 지원 범위·시간 미상·1908년 이전의 제한은 [계산 정책](./saju-hour-pillar-audit.md)을 따른다.
...
{
  "contextVersion": 1,
  "calculationPolicyVersion": "kr-mean-solar-midnight-v2",
