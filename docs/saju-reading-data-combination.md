...
> 현재 동작: [세 자료 조합 및 포츈텔러 분석](./fortuneteller-analysis.md), [KASI 연결](./wealth-ranking-kasi.md). 아래 내용은 초기 구현 기록을 포함한다. 현재 프롬프트는 v6이며 설치된 fortuneteller 원본 분석을 개인별 전체/부분 범위로 사용한다. 아래 과거 자체 구현 설명은 현재 실행 경로가 아니다.
