 ...
 * 金克木 (금극목): 금이 나무를 이긴다
 */
export const WUXING_DESTRUCTION = {
    목: '토',
    화: '금',
    토: '수',
    금: '목',
 ...
 * 두 오행이 상극 관계인지 확인
 */
export function isDestroying(from, to) {
    return WUXING_DESTRUCTION[from] === to;
}
/**
 * 두 오행의 관계 분석
 */
export function analyzeWuXingRelation(from, to) {
    if (from === to)
        return 'same';
 ...
 */
export function analyzeWuXingBalance(counts) {
 ...
 */
export function analyzeElementInteraction(from, to) {
    if (from === to) {
 ...
 * 특정 오행을 생(生)하는 오행 반환
 */
export function getGeneratingElement(element) {
 ...
 * 특정 오행을 극(克)하는 오행 반환
 */
export function getControllingElement(element) {
    const reverseDestruction = {
        목: '금',
        화: '수',
        토: '목',
        금: '화',
 ...
 * 특정 오행이 극(克)하는 오행 반환
 */
export function getControlledElement(element) {
    return WUXING_DESTRUCTION[element];
}
/**
 * 특정 오행을 설(洩)하는 오행 반환 (생하는 대상)
 */
export function getWeakeningElement(element) {
    return WUXING_GENERATION[element];
}
