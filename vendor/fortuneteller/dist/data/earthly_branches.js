 ...
 * 지지 인덱스로 지지 데이터 가져오기
 */
export function getEarthlyBranchByIndex(index) {
 ...
 */
export function getEarthlyBranchByKorean(korean) {
    return EARTHLY_BRANCHES.find((branch) => branch.korean === korean);
}
/**
 * 지지 한자로 지지 데이터 가져오기
 */
export function getEarthlyBranchByHanja(hanja) {
    return EARTHLY_BRANCHES.find((branch) => branch.hanja === hanja);
}
/**
 * 연도로 띠(지지) 계산하기
 */
export function getAnimalSignByYear(year) {
    // 1900년은 자(쥐)년 기준
    const baseYear = 1900;
 ...
 * 삼합 체크 함수
 */
export function checkSamHap(branches) {
 ...
 * 삼형 체크 함수
 */
export function checkSamHyeong(branches) {
    const branchSet = new Set(branches);
    const hyeongList = [];
    // 무은지형 체크
 ...
 * 육해 체크 함수
 */
export function checkYukHae(branches) {
    const branchSet = new Set(branches);
    const haeList = [];
 ...
 * 지지 관계 종합 분석
 */
export function analyzeBranchRelations(branches) {
 ...
 */
export function extractJiJangGan(branch) {
    const jiJang = JI_JANG_GAN[branch];
    const stems = [jiJang.primary];
    if (jiJang.secondary)
        stems.push(jiJang.secondary);
    if (jiJang.residual)
        stems.push(jiJang.residual);
    return stems;
}
/**
 * 지장간 세력 계산 (절기 기준)
 ...
 */
export function calculateJiJangGanStrength(branch, monthIndex // 0-11 (0=입춘~, 1=경칩~, ...)
) {
    const jiJang = JI_JANG_GAN[branch];
    // 지지와 월령의 관계로 세력 결정
    const branchIndex = EARTHLY_BRANCHES.findIndex((b) => b.korean === branch);
    // monthIndex starts at 寅, whereas EARTHLY_BRANCHES starts at 子.
    const monthDiff = ((monthIndex + 2) - branchIndex + 12) % 12;
 ...
 * 극을 받으면 실령(失令)
 */
export function checkWolRyeong(dayStem, monthBranch) {
