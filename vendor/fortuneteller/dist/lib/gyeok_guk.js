 ...
 * 격국 판단 메인 함수
 */
export function determineGyeokGuk(sajuData) {
    // 1. 특수 격국 판단 (종격)
    ...
    // 월지의 정기(주기)로 십성 판단
    let dominantTenGod = null;
    // 십성 분포에서 가장 많은 십성 찾기
    if (sajuData.tenGodsDistribution) {
        const tenGodEntries = Object.entries(sajuData.tenGodsDistribution);
 ...
 * 특수 격국(종격) 판단
 */
function checkSpecialGyeokGuk(sajuData) {
 ...
 * 십성을 격국으로 매핑
 */
function mapTenGodToGyeokGuk(tenGod) {
    if (!tenGod)
        return 'balanced';
    const gyeokGukMap = {
        '정관': 'jeong_gwan',
 ...
 * 격국 정보 조회
 */
export function getGyeokGukInfo(gyeokGuk) {
