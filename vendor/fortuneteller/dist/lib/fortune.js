 ...
 * 사주를 기반으로 운세 분석
 */
export function analyzeFortune(sajuData, analysisType, _targetDate) {
 ...
 * 종합 운세 분석
 */
function analyzeGeneralFortune(sajuData, balance) {
    const positiveParts = [];
    const negativeParts = [];
    const adviceParts = [];
    // 오행 균형 분석
    if (balance.balanced) {
        ...
        if (balance.strong.length > 0) {
            negativeParts.push(`사주 내 ${balance.strong.join(', ')} 기운이 과도하게 강하여 오행의 불균형이 나타나고 있습니다.`);
            adviceParts.push(`이를 보완하기 위해서는 ${balance.weak.join(', ')} 기운을 보충하는 활동이나 환경을 만들어가는 것이 중요합니다.`);
        }
    }
    ...
    if (balance.strong.length > 0) {
        ...
        if (balance.strong.length > 1) {
            positiveParts.push(`${strongDesc}이 복합적으로 나타나 다채롭고 입체적인 성격을 형성하고 있습니다.`);
        }
        else {
    ...
    if (sajuData.tenGodsDistribution) {
        ...
        const tenGodsStrengths = [];
        const tenGodsWeaknesses = [];
        const tenGodsAdvice = [];
        tenGodsInterpretations.forEach((interp) => {
            if (interp.count > 0) {
                // 강도에 따라 표현 조정
                const intensityPrefix = interp.intensity === 'very_strong'
                    ? '매우 강한'
                ...
                if (interp.weaknesses.length > 0 && interp.count >= 1.5) {
                    const weaknessText = interp.intensity === 'very_strong'
 ...
 * 직업운 분석
 */
function analyzeCareerFortune(sajuData, balance) {
    const positiveParts = [];
    const negativeParts = [];
    const adviceParts = [];
    // 일간 기준 직업 적성
    ...
    if (sajuData.tenGodsDistribution) {
        ...
        if (dist.비견 + dist.겁재 >= 4) {
            negativeParts.push('비겁이 과다하게 나타나 독립적인 성향은 강하지만 타인과의 협업에서 어려움을 겪을 수 있습니다.');
            adviceParts.push('팀워크 능력을 개발하는 노력을 기울이거나, 혹은 독립 사업을 통해 자신만의 길을 개척하는 것도 좋은 선택이 될 수 있습니다.');
        }
    }
 ...
 * 재물운 분석
 */
export function analyzeWealthFortune(sajuData, balance = analyzeWuXingBalance(sajuData.wuxingCount)) {
    const positiveParts = [];
    const negativeParts = [];
    const adviceParts = [];
    // 재성(財星) 분석 - 일간과 극하는 오행
    const dayStemElement = sajuData.day.stemElement;
    const wealthElement = getDestroyedElement(dayStemElement);
    const wealthCount = sajuData.wuxingCount[wealthElement];
    if (wealthCount >= 2) {
        positiveParts.push(`사주 내 재성(${wealthElement})이 충분히 자리잡고 있어 재물을 모으고 불리는 운이 양호한 편입니다.`);
        adviceParts.push('재물운이 뒷받침되는 시기이므로 투자 기회가 찾아올 때 적극적으로 검토하고 도전해보는 것이 좋습니다.');
    }
    else if (wealthCount === 1) {
        positiveParts.push('재성이 적당히 배치되어 있어 안정적인 재물운을 보이고 있습니다.');
        adviceParts.push('무리한 투자로 위험을 감수하기보다는 꾸준한 저축과 안정적인 재테크를 통해 자산을 축적하는 것이 유리합니다.');
    }
    else {
        ...
        adviceParts.push('불필요한 지출을 줄이고 수입원을 다양화하는 노력이 필요하며, 장기적인 재무 계획을 세우는 것이 중요합니다.');
    }
    // 십성 기반 재물운 분석
    if (sajuData.tenGodsDistribution) {
        const dist = sajuData.tenGodsDistribution;
        ...
        if (dist.편재 >= 2) {
            ...
            tenGodsAdvice.push('적극적으로 투자 기회와 사업 아이템을 모색하되, 리스크 관리와 손절 기준을 명확히 하여 위험에 대비해야 합니다');
        }
        // 식신: 생재(生財), 재물을 만드는 능력
        if (dist.식신 >= 2) {
            tenGodsParts.push('식신이 강하여 스스로 재물을 창출하고 만들어내는 능력이 뛰어납니다');
            tenGodsAdvice.push('창의적인 아이디어나 콘텐츠 제작, 자신만의 특기를 활용한 사업으로 수익을 창출하는 방법을 고려해보세요');
        }
        if (tenGodsParts.length > 0) {
        ...
        if (dist.비견 + dist.겁재 >= 3) {
            if (dist.정재 + dist.편재 === 0) {
                ...
                adviceParts.push('다른 사람과의 협업보다는 독립적인 수입원을 개발하고 자신만의 사업 모델을 만드는 데 집중하는 것이 좋습니다.');
            }
            else {
                negativeParts.push('비겁이 많아 재물을 여러 사람과 나누거나 경쟁 속에서 획득해야 하는 경우가 발생할 수 있습니다.');
                adviceParts.push('재물 관리를 철저히 하고 충동적인 지출이나 불필요한 소비를 자제하여 자산을 보호하는 것이 중요합니다.');
            }
        }
    }
 ...
 * 건강운 분석
 */
function analyzeHealthFortune(_sajuData, balance) {
    const positiveParts = [];
    const negativeParts = [];
    const adviceParts = [];
    if (balance.balanced) {
        positiveParts.push('오행의 기운이 조화롭게 균형을 이루고 있어 전반적인 건강운이 양호하며 큰 질병에 대한 저항력도 강한 편입니다.');
        adviceParts.push('건강한 체질을 유지하기 위해 규칙적인 생활 리듬과 적절한 운동, 균형잡힌 식사를 지속하는 것이 중요합니다.');
    }
    else {
        const strongParts = [];
        const weakParts = [];
        ...
        if (healthAdvice.length > 0) {
            adviceParts.push(`건강 관리를 위해 ${healthAdvice.join(', ')}에 집중하는 것이 좋습니다. 정기적인 건강 검진을 통해 예방에 힘쓰는 것도 중요합니다.`);
        }
    }
 ...
 * 애정운 분석
 */
function analyzeLoveFortune(sajuData, _balance) {
    const positiveParts = [];
    const negativeParts = [];
    const adviceParts = [];
    const dayStemElement = sajuData.day.stemElement;
    ...
    switch (dayStemElement) {
        case '목':
            positiveParts.push('일간의 목(木) 기운으로 따뜻하고 배려심이 깊은 연애 스타일을 가지고 있어 상대방에게 안정감과 위로를 줍니다.');
            adviceParts.push('파트너에게 성장의 기회를 주고 함께 발전해나가는 관계를 만들면 더욱 행복한 사랑을 할 수 있습니다.');
            break;
        case '화':
            positiveParts.push('화(火) 일간의 영향으로 열정적이고 표현이 풍부한 사랑을 하며, 상대방에게 강렬한 인상을 남깁니다.');
            adviceParts.push('때로는 차분하고 절제된 모습도 보여주면 관계가 더욱 깊어지고 오래 지속될 수 있습니다.');
            break;
        case '토':
            positiveParts.push('토(土) 일간으로 안정적이고 헌신적인 관계를 추구하며, 신뢰와 책임감을 바탕으로 사랑을 쌓아갑니다.');
            adviceParts.push('너무 보수적이거나 변화를 두려워하지 말고 가끔은 새로운 시도로 관계에 활력을 불어넣는 것도 좋습니다.');
            break;
        case '금':
            positiveParts.push('금(金) 일간의 특성으로 진지하고 책임감있는 태도로 관계에 임하며, 약속과 신의를 중요시합니다.');
            adviceParts.push('융통성과 여유를 가지고 상대방의 감정을 이해하려는 노력을 기울이면 관계가 더욱 원만해질 것입니다.');
            break;
        case '수':
            positiveParts.push('수(水) 일간으로 깊이있고 지적인 교감을 중시하며, 정신적인 유대감과 이해를 바탕으로 사랑을 키워갑니다.');
    ...
    if (sajuData.tenGodsDistribution) {
        ...
        if (isMale) {
            ...
            if (spouseCount >= 2) {
                ...
                if (dist.정재 >= 1) {
                    spouseAdvice.push('정재의 영향으로 안정적이고 진실한 결혼 운을 가지고 있어 좋은 배우자를 만날 가능성이 높습니다');
                }
                if (dist.편재 >= 2) {
            ...
            if (spouseCount >= 2) {
                ...
                if (dist.정관 >= 1) {
                    spouseAdvice.push('정관의 작용으로 안정적이고 존경할 수 있는 배우자와 행복한 결혼 생활을 이어갈 가능성이 높습니다');
                }
                if (dist.편관 >= 2) {
        ...
        if (dist.식신 + dist.상관 >= 4) {
            if (isMale) {
                negativeParts.push('식상이 과다하여 재성을 생하는 효과는 있으나 배우자와의 의견 충돌이나 갈등이 발생할 가능성이 있습니다.');
            }
            else {
                negativeParts.push('식상이 과다하여 관성을 극하는 작용으로 인해 배우자 인연에 어려움이 있거나 관계 유지에 주의가 필요합니다.');
            }
            adviceParts.push('상대방을 존중하고 이해하려는 마음가짐을 가지며, 배려와 소통을 통해 갈등을 예방하는 노력이 중요합니다.');
        }
        // 비겁(比劫) 과다: 배우자성 경쟁
        if (dist.비견 + dist.겁재 >= 4) {
            negativeParts.push('비겁이 많아 이성 관계에서 경쟁 상황이 발생하거나 애정 문제로 인한 갈등을 겪을 수 있습니다.');
            adviceParts.push('독점욕이나 질투심을 줄이고 상대방의 자유와 개성을 존중하며 신뢰를 바탕으로 관계를 발전시켜 나가세요.');
        }
    }
 ...
 * 날짜 기반 시드 생성 함수
 */
function generateDateSeed(date, sajuData) {
 ...
 * 일일 운세 생성
 */
export function getDailyFortune(sajuData, date) {
 ...
 * 오행이 극하는 오행 반환
 */
function getDestroyedElement(element) {
    const map = {
        목: '토',
        화: '금',
        토: '수',
