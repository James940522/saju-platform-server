 ...
 */
export function calculateTenGodsDistribution(sajuData) {
    const distribution = {
        비견: 0,
        겁재: 0,
        식신: 0,
    ...
    // 연주, 월주, 시주의 천간 (일주 제외)
    const stems = [sajuData.year.stem, sajuData.month.stem, ...(sajuData.hour ? [sajuData.hour.stem] : [])];
    stems.forEach((stem) => {
        // Only the day-master position is excluded; peers elsewhere are evidence.
    ...
    if (sajuData.jiJangGan) {
        ...
        const branches = [
            ...
            sajuData.day.branch,
            ...(sajuData.hour ? [sajuData.hour.branch] : []),
        ];
 ...
 * TODO: 지장간 구현 시 상세화
 */
function mapBranchToStem(branch) {
    const mapping = {
        자: '계',
        축: '기',
        인: '갑',
 ...
 */
export function generateTenGodsList(sajuData) {
    const dayStem = sajuData.day.stem;
    const tenGods = [];
    // 연주 천간
    ...
    tenGods.push('비견');
    // 시주 천간
    if (sajuData.hour && sajuData.hour.stem !== dayStem) {
        tenGods.push(calculateTenGod(dayStem, sajuData.hour.stem));
    }
    else if (sajuData.hour) {
        tenGods.push('비견');
    }
    // 지지는 생략 또는 간단히 추가 가능
    return tenGods;
 ...
 * 십성별 의미 해석 (연속적 범위 반영)
 */
export function interpretTenGod(tenGod, count) {
    const strengths = [];
    const weaknesses = [];
    const advice = [];
 ...
 * 사주의 모든 십성 해석
 */
export function interpretAllTenGods(distribution) {
