...
export function selectYongSin(sajuData) {
    ...
    const recommendations = generateRecommendations(primaryYongSin, secondaryYongSin, jiSin, leapMonthAnalysis);
    return {
        primaryYongSin,
        secondaryYongSin,
        xiSin,
 ...
 * 나를 생(生)하는 오행
 */
function getShengMeElement(element) {
    const shengMeMap = {
        목: '수', // 수생목
        화: '목', // 목생화
        토: '화', // 화생토
 ...
 * 나를 극(克)하는 오행
 */
function getKeMeElement(element) {
    const keMeMap = {
        목: '금', // 금극목
        화: '수', // 수극화
        토: '목', // 목극토
 ...
 * 가장 약한 오행 찾기
 */
function findWeakestElement(sajuData) {
 ...
 */
function generateRecommendations(primaryYongSin, secondaryYongSin, jiSin, leapMonthAnalysis) {
 ...
 */
export function generateYongSinAdvice(yongSin) {
    const advice = [];
    ...
    if (yongSin.leapMonthAnalysis && yongSin.leapMonthAnalysis.isLeapMonth) {
        ...
        if (yongSin.leapMonthAnalysis.specialCharacteristics.length > 0) {
            advice.push(`특별한 성향: ${yongSin.leapMonthAnalysis.specialCharacteristics.slice(0, 2).join('. ')}`);
        }
        if (yongSin.leapMonthAnalysis.elementAdjustments.length > 0) {
            const adjustment = yongSin.leapMonthAnalysis.elementAdjustments[0];
            if (adjustment) {
                advice.push(`오행 조정: ${adjustment.element} ${adjustment.originalStrength} → ${adjustment.adjustedStrength} (${adjustment.reason})`);
            }
        }
    }
    if (yongSin.recommendations.colors.length > 0) {
        advice.push(`길한 색상: ${yongSin.recommendations.colors.slice(0, 3).join(', ')}을 활용하세요`);
    }
    if (yongSin.recommendations.directions.length > 0) {
        advice.push(`유리한 방향: ${yongSin.recommendations.directions.join(', ')} 방향이 길합니다`);
    }
    if (yongSin.recommendations.careers.length > 0) {
        advice.push(`적합한 직업: ${yongSin.recommendations.careers.slice(0, 4).join(', ')} 등`);
    }
    if (yongSin.recommendations.activities.length > 0) {
        advice.push(`권장 활동: ${yongSin.recommendations.activities.slice(0, 3).join(', ')} 등`);
    }
    ...
    if (yongSin.leapMonthAnalysis && yongSin.leapMonthAnalysis.isLeapMonth) {
        if (yongSin.leapMonthAnalysis.recommendations.length > 0) {
            advice.push(`윤달 특별 권장: ${yongSin.leapMonthAnalysis.recommendations.slice(0, 2).join('. ')}`);
        }
    }
    return advice;
}
