 ...
 * 도화살 판단 (지지 조합)
 */
function checkDoHwaSal(branches) {
    const branchSet = new Set(branches);
    // 인오술 → 묘
    if ((branchSet.has('인') || branchSet.has('오') || branchSet.has('술')) &&
        branchSet.has('묘')) {
        return true;
    }
    // 사유축 → 오
    if ((branchSet.has('사') || branchSet.has('유') || branchSet.has('축')) &&
        branchSet.has('오')) {
        return true;
    }
    // 신자진 → 유
    if ((branchSet.has('신') || branchSet.has('자') || branchSet.has('진')) &&
        branchSet.has('유')) {
        return true;
    }
    // 해묘미 → 자
    if ((branchSet.has('해') || branchSet.has('묘') || branchSet.has('미')) &&
        branchSet.has('자')) {
        return true;
    }
    return false;
 ...
 * 역마살 판단 (지지 조합)
 */
function checkYeokMaSal(branches) {
    const branchSet = new Set(branches);
    // 인오술일주 → 신
    if ((branchSet.has('인') || branchSet.has('오') || branchSet.has('술')) &&
        branchSet.has('신')) {
 ...
 * 공망 판단 (일주 기준 60갑자 순환)
 */
function checkGongMang(dayBranch, branches) {
    const gongMangTable = {
        자: ['술', '해'],
        축: ['술', '해'],
        인: ['자', '축'],
 ...
 * 화개살 판단 (지지 조합)
 */
function checkHwaGaeSal(branches) {
    const branchSet = new Set(branches);
    // 인오술 → 술
    if ((branchSet.has('인') || branchSet.has('오') || branchSet.has('술')) &&
        branchSet.has('술')) {
 ...
 * 사주에서 신살 찾기
 */
export function findSinSals(sajuData) {
    const sinSals = [];
 ...
 */
function checkWonJinSal(branches) {
    const branchSet = new Set(branches);
    const chungPairs = [
        ['자', '오'],
        ['축', '미'],
 ...
 */
function checkGwiMunGwanSal(branches) {
    const branchSet = new Set(branches);
    const gwiMunBranches = ['인', '신', '사', '해'];
 ...
 * 신살 정보 조회
 */
export function getSinSalInfo(sinSal) {
    return {
        sinSal,
        ...SIN_SAL_DATA[sinSal],
    };
 ...
 * 신살 기반 특수 해석
 */
export function interpretBySinSal(sinSals) {
    const warnings = [];
    const blessings = [];
    const specialAdvice = [];
 ...
 * 모든 신살 정보 가져오기
 */
export function getAllSinSalInfo(sinSals) {
